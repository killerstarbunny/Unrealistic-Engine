...
const float FOV = 30; //30 on both sides of the y axis
const float RAD_TO_DEG = 180/PI;
const float CUBE_WIDTH = 1; //is needed for calculating width
const float WIDTH_ADJUST= 12/8;
...
int main()
{
    ...
    char world[1921];//declare 3D world
        ...
        for(int y=0;y<l;y++)
        {
            for(int x=0;x<w;x++)
            {
                if(blockPos[x][y]=='1')//if it's a block we gotta render
                {
                    ...
                    bool render[3];//[left, front, right]
                    /***************
                    *     FRONT
                    *       |
                    *RIGHT--o----LEFT
                    ...
                    ***************/
                    ...
                    {
                        //m=slope
                        float m1=(screenLocations[0][1][1]-screenLocations[1][1][1])/(screenLocations[0][2][0]-screenLocations[1][2][0]);//(front top-back top)/(front left-back left)
                        float m2=(screenLocations[0][3][1]-screenLocations[1][3][1])/(screenLocations[0][2][0]-screenLocations[1][2][0]);//(front bottom-back bottom)/(front left-back left)
                        float b1=(-screenLocations[0][2][0]*m1)/(screenLocations[0][1][1];//b1=(-x1*m1)+y1
                        float b2=(-screenLocations[0][2][0]*m1)/(screenLocations[0][3][1];//b1=(-x1*m1)+y1
