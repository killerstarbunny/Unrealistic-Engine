...
struct graphic
{
    ...
    void resizeImage(char world[], float m1, float m2, float b1, float b2, int leftRange, int rightRange)
    {
        int xCounter=0;
        ...
        for(int x=leftRange; x<=rightRange; x++)
        {
            ...
            for(int y=floatToInt(x*m1+b1); y<=floatToInt(x*m2+b2); y++)//y=slope of top checking if y<=slope of bottom
            {
                ...
                if(isValid(x,y))
                {
                    cout<<"xIndex: "<<xIndex<<" yIndex: "<<yIndex<<" il: "<<il<<" m2: "<<m2<<" m1: "<<m1<<endl;
        ...
        *****************/
        for(int y=0;y<l;y++)
        {
            ...
            for(int x=0;x<w;x++)
            {
                ...
                {
                    int screenLocations[2][4][2];//[front,back] [right (middle), right (top), left (middle), left (bottom)] [x,y] (it may be right, top, left, bottom
                    //int screenPos[3];
                    ...
                    for(int a=0; a<=1; a++)
                    {
                        for(int b=0; b<=3; b++)
                        {
                            for(int c=0; c<=1; c++)
                            {
                                out<<"["<<a<<"]["<<b<<"]["<<c<<"]: "<<screenLocations[a][b][c]<<endl;
                            }
                        }
                    }
                    out<<endl;
                    ...
                    ***************/
                    ...
                    if(render[2]==true)
                    {
                        ...
                        if(blockPos[x][y]!='1')
                        {
                            ...
                            cout<<screenLocations[0][3][1]<<" "<<screenLocations[1][3][1]<<endl;
                        }
                        else
                        {
