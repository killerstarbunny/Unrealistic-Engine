        ...
        for(int y=0;y<l;y++)
        {
            ...
            for(int x=0;x<w;x++)
            {
                if(blockPos[x][y]=='1')//if it's a block we gotta render
                {
                    ...
                    distances[0][0][1]=(playerY+(CUBE_WIDTH*.5))-(y+(CUBE_WIDTH*.5));//front right y
                    distances[0][1][0]=(playerY+(CUBE_WIDTH*.5))-(y+(CUBE_WIDTH*.5));//front top x
                    ...
                    distances[0][2][1]=(playerY+(CUBE_WIDTH*.5))-(y+(CUBE_WIDTH*.5));//front left y
                    distances[0][3][0]=(playerY+(CUBE_WIDTH*.5))-(y+(CUBE_WIDTH*.5));//front bottom x
                    distances[0][3][1]=CUBE_WIDTH*.5;
                    distances[1][0][0]=playerX-(x+(CUBE_WIDTH*.5));//back right x;
                    distances[1][0][1]=(playerY+(CUBE_WIDTH*.5))-(y-(CUBE_WIDTH*.5));//back right y;
                    distances[1][1][0]=(playerY+(CUBE_WIDTH*.5))-(y-(CUBE_WIDTH*.5));//back top x;
                    ...
                    distances[1][2][1]=(playerY+(CUBE_WIDTH*.5))-(y-(CUBE_WIDTH*.5));//back left y;
                    distances[1][3][0]=(playerY+(CUBE_WIDTH*.5))-(y-(CUBE_WIDTH*.5));//back bottom x;
                            ...
                            ***************************/
                            ...
                            else
                            {
                                ...
                                /*if(degree>FOV+90||degree<FOV-90)
                                {
                                    ...
                                    //Seems to work without it *shrug*
            ...
            if(playerY==y)//this would mean that the objects being rendered are behind the player, which is pointless to mess with
            {
                ...
                for(;;)
                {
                    ...
                    {
                        /*char **backupBlockPos= new char*[w];
                        for (int i = 0; i < l; i++) //
                        {
                            backupBlockPos[i] = new char[l];//gotta do this nonsense
                        }
                        for(int x=0; x<w; x++)
                        {
                            for(int y=0; y<l; y++)
                            {
                                blockPos[x][y]=backupBlockPos[x][y];
                            }
                        }
                        blockPos = new char*[l];
                        for (int i = 0; i < w; i++) //
                        {
                            blockPos[i] = new char[w];//gotta do this nonsense
                        }
                        for(int x=0; x<w; x++)
                        {
                            int xCounter=0;
                    ...
                    else if(c==101&&playerX<w&&(blockPos[playerX+1][playerY]=='0'||blockPos[playerX+1][playerY]=='2'))//e
                    {
                        playerX++;
                        break;
                    }
