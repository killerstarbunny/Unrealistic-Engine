...
int normalizeDirection(int i)
{
    ...
    i%=4;//will normalize it back down inside of the range. This also handles negative numbers. -1%4=3 West
    return i;
}
...
int main()
{
    ...
    cout<<"Done"<<endl;
    _getch();
    int direction=0;//0=north, 1=east, 2=south, 3=west. This will likely be changed
    for(;;)//start on north side
    {
        ...
        * ORGIN IS +
        *****************/
        int startingY=playerY-200;
        if(startingY<0)
        {
            startingY=0;
        }
        int startingX=playerX-200;
        if(startingX<0)
        {
            startingX=0;
        }
        for(int y=startingY;y<l;y++)
        {
            bool leave=false;
            for(int x=startingX;x<w&&playerX+200>x;x++)
            {
